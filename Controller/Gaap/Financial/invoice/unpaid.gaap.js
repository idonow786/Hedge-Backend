  ...
  try {
    ...
    if (status === "Paid") {
      ...
      try {
        ...
        if (
          ...
          (project.projectType === "ICV" ||
            project.projectType === "ICV+external Audit")
        ) {
          const icvCertificateBuffer = await generateICVCertificate(
            project,
            customer
          );
    ...
    res.status(200).json({
      message:
        status === "Paid"
          ? emailSent
            ? "Payment received and invoice sent"
            : "Payment received, but there was an issue sending the invoice email"
...
async function generateReceiptPDF(invoice, project, customer, payment) {
  ...
  // Add logo
  const logoUrl =
    "https://firebasestorage.googleapis.com/v0/b/tempproject-4cb9b.appspot.com/o/logo.png?alt=media";
  ...
  // Add company details
  page.drawText("GAAP Associates", {
    x: 50,
    y: page.getHeight() - 120,
    size: 16,
    font: helveticaBold,
  });
  page.drawText("Abu Dhabi, UAE", {
    x: 50,
    y: page.getHeight() - 140,
    size: 10,
    font: helvetica,
  });
  page.drawText("Phone: 02 650 2924 | Email: info@gaapaudit.com", {
    x: 50,
  ...
  // Add receipt details
  page.drawText(`Receipt #: ${invoice.invoiceNumber}`, {
    x: 400,
  ...
  page.drawText(customer.address.street, {
    x: 50,
    y: page.getHeight() - 235,
    size: 10,
    font: helvetica,
  });
  page.drawText(`${customer.address.city}, ${customer.address.country}`, {
    x: 50,
  ...
  currentY -= 25;
  page.drawText(`Project Name: ${project.projectName}`, {
    x: 50,
    y: currentY,
    size: 12,
    font: helvetica,
  ...
  currentY -= 20;
  page.drawText(
    `Amount Received: ${invoice.total.toFixed(2)} ${invoice.currency}`,
    { x: 50, y: currentY, size: 12, font: helveticaBold }
  );
  currentY -= 20;
  page.drawText(`Payment Method: Bank Transfer`, {
    x: 50,
    y: currentY,
    size: 12,
    font: helvetica,
  ...
  currentY -= 20;
  page.drawText(`Payment Date: ${new Date().toLocaleDateString()}`, {
    x: 50,
    y: currentY,
    size: 12,
    font: helvetica,
  ...
  currentY -= 40;
  page.drawText("Project Payment Summary:", {
    x: 50,
    y: currentY,
    size: 14,
    font: helveticaBold,
  ...
  currentY -= 25;
  page.drawText(
    `Total Project Amount: ${discountedAmount.toFixed(2)} ${payment.currency}`,
    { x: 50, y: currentY, size: 12, font: helvetica }
  );
  currentY -= 20;
  page.drawText(
    `Total Paid Amount: ${payment.paidAmount.toFixed(2)} ${payment.currency}`,
    { x: 50, y: currentY, size: 12, font: helvetica }
  );
  currentY -= 20;
  page.drawText(
    `Remaining Amount: ${remainingAmount.toFixed(2)} ${payment.currency}`,
    { x: 50, y: currentY, size: 12, font: helvetica }
  );
  currentY -= 20;
  page.drawText(`Payment Status: ${payment.paymentStatus}`, {
    x: 50,
    y: currentY,
    size: 12,
    font: helvetica,
  ...
  page.drawText("This receipt is proof of payment for the services rendered.", {
    x: 50,
    y: footerY - 15,
    size: 10,
    font: helvetica,
...
async function sendInvoiceEmail(
  ...
  const tempPdfPath = path.join(
    __dirname,
    `temp_receipt_${invoice.invoiceNumber}.pdf`
  );
...
async function sendICVCertificateEmail(
  fromEmail,
  toEmail,
  pdfBuffer,
  project,
  ...
  const tempPdfPath = path.join(
    __dirname,
    `temp_icv_certificate_${project._id}.pdf`
  );
  await fs.writeFile(tempPdfPath, pdfBuffer);
...
module.exports = {
  getProjectsWithInvoiceStatus,
  updatePayment,
  getProjectsWithPaymentStatus,
};
