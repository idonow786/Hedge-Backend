...
const generateAndSendProposal = async (req, res) => {
  try {
    ...
    const project = await GaapProject.findById(projectId).populate("customer").populate("products");
    if (!project) {
      console.log("Project not found");
      return res.status(404).json({ message: "Project not found" });
    }
    const projectProducts = await GaapProjectProduct.findOne({ project: project._id }).lean();
    ...
    const template1 = await fs.readFile(
      path.join(__dirname, "../Template/page1.html"),
      "utf8"
    );
...
//
    ...
    const data = {
      ...
      contactPerson: `${customer.contactPerson1.title || 'Mr'}. ${customer.contactPerson1.name}`,
    ...
    try {
      ...
      const mailOptions = {
        ...
        text: `Dear ${customer.contactPerson1.title || 'Mr'}. ${customer.contactPerson1.name},
