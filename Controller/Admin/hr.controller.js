...
const HRUser = require('../../Model/HR/HRUser');
const HRBusiness = require('../../Model/HR/HRBusiness');
...
const createUserAndBusiness = async (req, res) => {
  try {
    ...
    const existingBusiness = await HRBusiness.findOne({ BusinessName: businessName });
    if (existingBusiness) {
    ...
    // Create business
    const newBusiness = new HRBusiness({
...
const updateUserAndBusiness = async (req, res) => {
  try {
    ...
    const business = await HRBusiness.findOne({ AdminID: userId });
    if (!business) {
    ...
    if (updateData.BusinessName && updateData.BusinessName !== business.BusinessName) {
      const existingBusiness = await HRBusiness.findOne({ BusinessName: updateData.BusinessName });
      if (existingBusiness) {
        return res.status(400).json({ message: 'Business name already exists' });
      }
    }
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        apiResponse = await axios.post('https://chocolate-reindeer-649283.hostingersite.com/api/update-user', null, {
          params: {
            email: user.email,
...
const deleteUserAndBusiness = async (req, res) => {
  try {
    ...
    const deletedBusiness = await HRBusiness.findOneAndDelete({ AdminID: userId });
    if (!deletedBusiness) {
      return res.status(404).json({ message: 'Associated business not found' });
    }

    // Delete the user
    await HRUser.findByIdAndDelete(userId);

    // Make POST request to external API with retry logic
    const maxRetries = 3;
    for (let i = 0; i < maxRetries; i++) {
      try {
        await axios.post('https://chocolate-reindeer-649283.hostingersite.com/api/delete-user', null, {
          params: {
            id: businessPhpId
          },
          timeout: 10000 // 10 seconds timeout
  ...
  const getAllUsersWithBusinesses = async (req, res) => {
    try {
      ...
      const usersWithBusinesses = await Promise.all(users.map(async (user) => {
        ...
        const business = await HRBusiness.findOne({ AdminID: user._id });
